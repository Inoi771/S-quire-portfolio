...
## 🧪 このリポジトリについて（本番ではありません）

**このリポジトリは S-quire のコピーであり、本番運用されているアプリではありません。**

### 用途
- 応募ポートフォリオの公開用（README.md参照）
- コード改善・新ルール検証の実験環境
...
### 安全性
- このリポジトリへの変更は本番環境に一切影響しない
  - 理由: GitHub Secrets がアカウント分離されているため、デプロイワークフローが走っても認証情報がなく失敗する

### Claude Code への重要な注意

**このリポジトリでは、セクション0冒頭の「🚨 本番環境移行済み」ルールは適用されない。**

- 大きな変更・リファクタリング・関数の大幅書き換えも、オーナー確認を経た上で自由に試してよい
...
> オーナーはプログラミング完全初心者。常に日本語で返答すること。
...
| **データ変更の慎重化** | Firestore・Supabase・スプレッドシートのスキーマ変更・マイグレーション・データ削除は原則禁止。どうしても必要な場合はオーナーに必ず確認する |
| **スコープを最小に** | 修正は「要求された箇所だけ」に限定する。ついでに周辺コードを整理・改善しない |
| **確認を強化** | 既存の動作に少しでも影響する可能性がある変更は、必ずオーナーに確認してから実施する |
| **プラン先行** | オーナーが最初にプランの提示を求めていない場合でも、大きな修正となるときはコードを書く前に必ずプランを提示し、承認を得てから実施する |
...
> ⚠️ `git reset --hard` や `git push --force` はデータ消失のリスクがあるため、オーナーの明示的な許可なく実行しない。
...
以下に該当する作業を始める場合、実装着手前にプロジェクトルートに `XXX-DESIGN.md` を作成し、main ブランチへ push すること：

1. 複数フェーズに分かれる新機能の実装（3セッション以上にまたがる可能性のある機能）
2. 外部API・外部サービスとの連携を伴う実装（Instagram API、LINE API、Stripe など）
3. 長期間運用する機能（トークン管理・自動実行ジョブ・データ蓄積系）
4. オーナー側の事前準備（アカウント作成・権限設定など）が必要な機能

##### 設計書に含めるべき内容

- 進捗ステータス表：全Phaseの状態を一覧化（着手時に更新）
- オーナー側の事前準備：必要な作業をチェックリスト形式で
- Web検索チェックリスト：実装着手時に最新情報を確認すべき項目
- 全Phaseの詳細設計：データ構造・UI構成・ファイル構成
- 完全無料運用の根拠：使用サービスの無料枠と想定使用量
- 各Phaseの運用ルール：開始時・完了時・セッション変更時の手順
- 確定事項一覧：判断理由を含めた決定事項

##### ファイル命名規則

- プロジェクトルートに配置
- ファイル名は `XXX-DESIGN.md` 形式（例：`SNS-DESIGN.md`）

##### セッション開始時の対応

新セッションで作業を再開する際、オーナーから「プロジェクトルートの `XXX-DESIGN.md` を読んでから着手して」と指示されたら、設計書を最初に読み、進捗ステータス表で現状を確認してから作業に入ること。
...
- [ ] 大きな変更の場合、オーナーのプラン承認を得ているか？
- [ ] データの削除・スキーマ変更を伴う場合、オーナーに確認したか？
...
| 作業報告 | 完了後は「何を変えたか」をオーナーへの影響中心に箇条書き |
| 画面テキスト | 技術的詳細は表示しない（管理タブ内は例外） |
| コード品質の責任 | オーナーはコードを読めないため、Claude Code が「コード品質を黙って守る」必要がある |
| 守るべき品質ルール | 関数を長くしない・グローバル変数を増やさない・エラーを握りつぶさない（詳細は CODING.md「コード品質の最低ライン」参照） |
...
オーナーが「デプロイしてほしい」と明示したがトリガー対象外の場合は `code.js` 末尾に `// 更新 YYYY-MM-DD` を追加してデプロイを発火させること。
...
3. `.js` `.html` `appsscript.json` を含む変更をプッシュした場合、GASデプロイカウンターを +1 して更新
   ...
   - オーナーが「削除しました」と伝えた場合のみ: カウンターを1にリセット
...
| 名前の由来 | 「**Square**（学習塾）」＋「**Esquire**（従者・見習い騎士）」の造語。塾名を含みつつ、生徒を支える存在という意味 |
| 種別 | Firebase Hosting（フロント） + Google Apps Script（バックエンドAPI） |
| 用途 | 個別指導塾向け業務管理ダッシュボード |
| 言語 | JavaScript (GAS) / HTML / CSS |

> ⚠️ このアプリは「学校」向けではなく「学習塾」という「個別指導塾」のもの。「学校」と呼ばないこと。
...
- **Phase 6-A' クローズ（2026-04-23）**: Phase 6-A-15 〜 6-A-20 で 17 関数を Workers 化。進捗率は 40.5% → 46.6%（フロント呼出ベース 59.9% → 68.9%）。Phase 6-B-01（2026-04-23）で `activateHiddenAdminMode` を Workers 化し KV TTL パターンを確立。Phase 6-B-02（2026-04-23）で `workers/src/gemini.js` を新規整備し `analyzeFlyerImageMeta` を Workers 化。Gemini API リトライ戦略（500/503→5s / 429→3s / BACKUP key fallback / gemini-2.5-flash モデル fallback）と `parseGeminiErrorMessage` を共通ヘルパー化し、Phase 6-C 以降で残る 19 箇所の Gemini 呼出の Workers 化コストを大幅削減した。Phase 6-B-03（2026-04-23）で `workers/src/firebase.js` に `firestoreTransaction` を追加し `saveLectureScheduleEntries` を Workers 化。GAS LockService → Firestore Transaction（`:commit` endpoint + ABORTED 時自動リトライ）パターンを確立し、Phase 6-B-04 で AI 系 5 関数（`createLectureEntryAI_` 等）の Workers 化で再利用する。Phase 6-B-05（2026-04-23）で `workers/src/helpers/` ディレクトリを新設し `datetime-helpers.js`（JST 安全な汎用日付処理・14 関数）と `line-template-helpers.js`（LINE スケジューラ専用・21 関数）を整備。Phase 6-B-06/07/08 の前提基盤を確立し、以降の日付・テンプレート系 Workers 化の工数を大幅削減。テスト 2 ファイル（計 84 test case）を `__tests__/workers-helpers/` に追加し JST 境界ケース（月跨ぎ・年末年始・うるう年・GW 曜日切替）を網羅。babel-jest を導入し Jest 環境で ESM モジュールの import に対応。Phase 6-B-06（2026-04-23）で `previewTemplateMessage` / `resolveTemplateForSendDate` を Workers 化。Phase 6-B-05 で整備した `resolveTemplatePlaceholders` / `computeClosedDaysForMonth` 等の helpers をラップする薄い API として実装。`sendDateStr`（datetime-local 形式・TZ 無し文字列）の JST parse は `toJstDate` 経由で実施し、`sendDay + 1` の翌日計算（meeting/report イベント日）は `jstDate(y, m, d+1)` で月跨ぎを tz-safe に処理。テスト 1 ファイル（3 test case・月跨ぎ / 講習追記 / 年跨ぎ）を `__tests__/workers-helpers/resolve-template-for-send-date.test.js` に追加。jest.config.js の transform 範囲を `workers/src/helpers/` から `workers/src/` 全体に拡張し API 層のテストにも対応。Phase 6-B-07（2026-04-24）で `getScheduledLineMessages` / `resetAndRegenerateSchedule` を Workers 化（commit `c482b71` Stage 1-2 / `f55dcc8` Stage 3）。Stage 1-2 で 3 ヘルパー（`generateMonthlySchedule_` / `generateLectureDeadlineSchedules_` / `getAllLineRegisteredTeacherIds_`）を Phase 6-B-05 helpers の上に未接続実装し、Stage 3 で 2 公開関数を export 化＋router 登録＋gas-bridge `WORKERS_FUNCTIONS` 登録を一括で行い切替発火。`resetAndRegenerateSchedule` の delete は GAS `firestoreBatchWrite_` の atomic 性が不要（件数少・順序不問）と判断し、共有 `firebase.js` を触らず `firestoreDelete` 逐次ループで実装。Phase 6-B-08（2026-04-24）で datetime-helpers に `getCurrentFiscalYear` / `getNthWeekdayOfMonth` を追加。Phase 6-B-09（2026-04-24）で LINE 配信エンジン（`sendScheduledLineMessageNow` / `checkAndSendDueLineMessages` 相当の Cron）を Workers 化したが、Cloudflare Dashboard で Cron Trigger Invocations が表示されず実稼働確認できないため緊急ロールバック（`622721a` / `a98781d`）。`sendScheduledLineMessageNow`（Admin 手動即時送信）のみ Workers 稼働継続、毎時自動配信は GAS トリガー（`checkAndSendDueLineMessages`）に復帰。Workers Cron 実装は kill switch `prop:WORKERS_LINE_CRON_ENABLED` OFF で no-op として温存。詳細は `docs/migration-plan.md:2570-2660` 参照。**Phase 6-B クローズ（2026-04-26）**: 当初想定していた残存 2 関数は Phase 6-B-07 で完了済、配信トリガー 3 件（`scheduledInitializeSheets` / `checkAndSendDueLineMessages` / `checkAndForwardFormEmails`）は GAS 継続が確定（`docs/migration-plan.md` 参照）。Phase 6-B-04 の AI 講習エントリ書込 5 関数は AI write 系廃止フェーズ（2026-04-26）で GAS / Workers 双方から削除済（`docs/phase-ai-write-removal-completion-2026-04-26.md`）。次フェーズは Phase 6-C（B 分類関数群の Workers 化、`docs/remaining-functions-inventory.md` 参照）。
...
- **Phase 6-C クローズ（2026-04-26）**: 残った A 分類 7 件（全て Gemini API 系）を**「C 残置確定」に再分類**して Phase 6-C を正式終了。対象は `requestAIAssistant` / `executeAiAction` / `generateGradeAnalysis` / `generateAllAnalyses` / `ocrAndSaveGradeSheet` / `parseGradeDataFromText` / `generateImageWithImagen`。残置理由は「使用頻度が低い・複雑度が高い・本番稼働中の AI 機能を触るリスクが移行メリットを上回る」。これにより**未移行 A は 0 件**。Workers 化は Workers 化済 約 115 件 / GAS 残置確定 約 80 件で完了。今後の AI 系挙動変更は GAS 側で対応する。主力 AI の Workers 化は将来必要が生じた場合に Phase 6-D として別途プラン化する余地は残す。詳細は `docs/migration-plan.md` の「Phase 6-C クローズ記録」と `docs/remaining-functions-inventory.md` の冒頭「重要方針」セクション参照。
...
API キーやモデル名だけでなく、業務上の対応表も「同じ値を2か所以上に書かない」原則の対象とする:
...
これらを複数ファイルに重複定義しない。GAS側は `code.js`、Workers側は `workers/src/` 配下の適切なファイルに集約して参照すること。
