...
4. ユーザー側の事前準備（アカウント作成・権限設定など）が必要な機能

##### 設計書に含めるべき内容

- 進捗ステータス表：全Phaseの状態を一覧化（着手時に更新）
- ユーザー側の事前準備：必要な作業をチェックリスト形式で
...
新セッションで作業を再開する際、ユーザーから「プロジェクトルートの `XXX-DESIGN.md` を読んでから着手して」と指示されたら、設計書を最初に読み、進捗ステータス表で現状を確認してから作業に入ること。
...
| コード品質の責任 | ユーザーはコードを読めないため、Claude Code が「コード品質を黙って守る」必要がある |
