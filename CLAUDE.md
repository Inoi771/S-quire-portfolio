...
年度（Year）: "2024年度版" 形式（フォルダ名）
  └── 教科書（Textbook）: NewHorizon / Sunshine / 入試対策編
...
│   │   ├── 中学2年（シート）
│   │   ├── 中学3年（シート）
│   │   └── レッスン順序（シート、1行目: 1年/2年/3年）
│   ├── Sunshine.sheets（同上構成）
│   ├── 入試対策編.sheets
│   │   ├── 不規則動詞①（シート、14列）
│   │   ├── 不規則動詞②（シート、18列）
│   │   └── 通常（シート、7列）
...
（例: `2024年度版_NewHorizon_中学1年_単語.pdf`）
...
4. `.js` `.html` `appsscript.json` `.github/workflows/*.yml` を含む変更をプッシュした場合、**CLAUDE.md のデプロイカウンターを +1 して更新する**
   - カウンターが 180 以上の場合は下記の太字警告を CLAUDE.md に追記/維持し、ユーザーへ口頭でも「現在 **N回目** のデプロイです。⚠️ GAS プロジェクトの履歴が限界に近づいています。GAS エディタの左端のメニュー →『プロジェクト履歴』→ 下のごみ箱マークで削除してください。削除後に『削除しました』とお知らせください。」と必ず報告する
   - 180 未満の場合は警告不要
5. ユーザーが「削除しました」と伝えた場合のみ、カウンターを **1** にリセットし、警告コメントを削除して CLAUDE.md をプッシュする

#### デプロイカウンターが 180 以上になったときに追記する警告文（CLAUDE.md の「現在のデプロイ回数」直下に追記）
```
**⚠️ 警告: デプロイ回数が180回に達しました。GASプロジェクトの履歴を削除してください！**
**GAS エディタ → 左端のメニュー →「プロジェクト履歴」→ 下のごみ箱マークで削除してください。**
