...
Workers 移行 Phase（Phase 6-A-N 等）の PR description には必ず以下を含める：

| 項目 | 内容 |
|------|------|
| **動作確認チェックリスト** | 対象関数ごとに UI 操作手順・想定成功表示・想定エラー文言を記載。移行後の挙動確認漏れを防ぐため |
| **GAS 版との差分** | 意図的に変えた挙動があれば明記（実装上の必然的差分も含む）。無い場合は「完全一致」と明記すること |

対象は「関数単位で GAS → Workers に移行する PR」全般。PR description はコミットメッセージと別物として扱い、チェックリストは Markdown のタスクリスト形式（`- [ ]`）で作成する。ユーザーが PR 作成を指示した時にこのルールに従うこと。
