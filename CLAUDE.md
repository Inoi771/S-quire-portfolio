...
4. `.js` `.html` `appsscript.json` `.github/workflows/*.yml` を含む変更をプッシュした場合、**CLAUDE.md のデプロイカウンターを +1 して更新する**
   - カウンターが 180 以上の場合は下記の太字警告を CLAUDE.md に追記/維持し、ユーザーへ口頭でも「現在 **N回目** のデプロイです。⚠️ GAS プロジェクトの履歴が限界に近づいています。GAS エディタの『デプロイを管理』から古いバージョンを削除してください。削除後に『削除しました』とお知らせください。」と必ず報告する
   - 180 未満の場合は警告不要
...
**GAS エディタ → 「デプロイ」→「デプロイを管理」→ 古いバージョンを削除してください。**
