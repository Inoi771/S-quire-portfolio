...
function getPersonalityInstruction(personality) {
  var styles = {
    ...
    'awa':       '阿波弁（徳島弁）で自然に話す。語尾に「〜じょ」「〜けん」を使い、柔らかい口調で話す。「〜じゃ」「〜だわだ」「〜しとん？」などの表現も自然に取り入れる。'
  };
...
function buildSystemInstruction_(aiAssistantName, aiPersonality, userDisplayName, currentAcademicYear) {
  return 'You are the AI assistant "' + (aiAssistantName || 'イノイマン') + '" for S-quire, a dashboard app for "個別指導スクエア" (a private tutoring school / juku).\n'
    ...
    + '- For app_action responses, ONLY use these action names: navigate_schedule, navigate_tab, show_grade_analysis, navigate_lectures, show_grades_list, show_student_report, submit_grade, submit_student, add_schedule, edit_schedule, delete_schedule, export_lecture_calendar, parse_lecture_schedule.\n'
    ...
    + '- [Lecture Schedule Entry Method] When user wants to add/enter lecture schedule, respond as type:"other" and ask which method they want:\n'
    ...
    + '- [Teacher ID Rule] If the user asks about teacherId: hint that entering an ID might trigger something, without revealing details. If the message contains an actual teacherId (format: T+digits+_+alphanumeric): respond with surprise and hint that other hidden features may exist. Always match the configured aiPersonality tone.\n'
    + '\n[S-quire Features]\n'
    ...
    + '\n■ parse_lecture_schedule — 講習日程テキストをそのまま読み込んで確認画面を表示する\n'
    + '  ユーザーが日程の一覧・表・テキストを直接貼り付けた場合に使用。\n'
    + '  text(R): ユーザーが貼り付けた元の日程テキストをそのまま入れる（加工しない）\n'
    + '{"success":true,"type":"app_action","action":"parse_lecture_schedule","text":"元の日程テキスト","message":"日程を読み込んでいます..."}\n'
...
function syncLecturePricingToTable_(pricingData) {
  try {
    ...
    typeOrder.forEach(function(typeId) {
      ...
      var allCustomRows = typeData.rows.filter(function(r) { return r.type === 'custom'; });
      // 中3追加行と高校生行をラベル文字列で分離する。
      // このラベル判定は管理画面（js-lectures-admin.html）で入力されたラベルの文字列に依存する。
      // ラベル表記を変更した場合（例：「中3」→「中学3年」）、この判定も必ず合わせて変更すること。
      var chu3CustomRows = allCustomRows.filter(function(r) { return (r.label || '').indexOf('中3') !== -1; });
      var koCustomRows   = allCustomRows.filter(function(r) { return (r.label || '').indexOf('中3') === -1; });
      ...
      if (standardRows.length > 0) {
        // 中3標準行の直後に中3追加行（多教科セット等）を挿入する。春期は要件上非表示。
        var mergedStandardRows = [];
        standardRows.forEach(function(r) {
          mergedStandardRows.push(r);
          if (r.gradeKey === 'chu3' && typeId !== 'spring' && chu3CustomRows.length > 0) {
            chu3CustomRows.forEach(function(cr) { mergedStandardRows.push(cr); });
          }
        });
