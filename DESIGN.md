...
## Firestore 読み取り最小化の原則

Firestore は読み取り回数を最小化する設計を必ず検討すること。

**基本方針:**
- 書き込み時に集計ドキュメントも同時更新し、読み取り時は集計ドキュメント1件で済むようにする
- 書き込みが +1回 増えても、読み取りが数百回→1回に減るなら十分見合う
- サブコレクション＋親ドキュメントに集計を持たせる構造が有効

**具体例（成績データの場合）:**
- 個別成績はサブコレクションに格納（個別更新しやすい）
- 校舎平均・受験者数などの集計は親ドキュメントに持たせる（読み取り専用）
- 年度一覧はメタドキュメント1件で管理する（全件取得を避ける）

**実施済みの最適化:**
- 成績表の全生徒プリフェッチを廃止 → タブクリック時のオンデマンド読み込みに変更（N×2回 → 2回）
- schoolAveragesのテスト件数分の個別取得を `.where('year')` による1回の一括クエリに変更（N回 → 1回）
- 講習管理タブの30秒自動更新にメインタブ・ブラウザタブの非表示ガードを追加（240回/時間 → 0回）

**注意:**
- 1ドキュメントの上限は 1MB。全件を1ドキュメントにまとめるのは不可
- 頻繁に個別更新されるデータは1ドキュメントにまとめない（競合・上書きリスク）
