...
| submitStudentInfo | 書込 | students(Supabase) | フロント: js-grades | なし | **A** | **高** | **未移行**。生徒登録・LockService 使用（移行時に要対応） |
...
| ocrAndExtractAverages | 読取 | Gemini API | フロント: js-grades-list | GEMINI_API_KEY | **A** | **中** | **未移行**。OCR・平均点抽出 |
...
| ocrLectureSchedule | 書込 | Gemini API | フロント: js-lectures | GEMINI_API_KEY | **A** | **中** | **未移行**。OCR 機能（Gemini Workers helper 流用可） |
...
| 分類 | 件数 | 内訳 |
|------|------|------|
| ✅ Workers 化済み | 約 110 | 旧台帳掲載 94 + 旧台帳未掲載 16 |
| **未移行 A** | **9** | submitStudentInfo, ocrAndSaveGradeSheet, parseGradeDataFromText, parseAndSaveAveragesFromText, ocrAndExtractAverages, generateGradeAnalysis, generateAllAnalyses, requestAIAssistant, executeAiAction, ocrLectureSchedule, parseLectureScheduleFromText, generateImageWithImagen, transcribeAndSummarizeAudio, mergeTranscriptsAndSummarize（実 14 件・うち AI 系 12 件） |
...
| 関数 | 場所 | 主な障壁 | 優先度 |
|------|------|---------|--------|
| `submitStudentInfo` | students.js | LockService（ID 採番） | 高 |
...
| `ocrAndExtractAverages` | students.js | Gemini API | 中 |
...
| `ocrLectureSchedule` | features.js | Gemini API | 中 |
...
### 次の Workers 化候補（Phase 6-C-02 以降）

未移行 A 分類 14 件（上記「未移行 A 分類の詳細」表参照）から選定。**優先度「高」3 件**：

1. **`submitStudentInfo`**（students.js）
   - 障壁: LockService（ID 採番の atomicity）
   - 解法案: Postgres UNIQUE 制約 + PK 違反時リトライ（最大 3 回）/ または `firestoreTransaction` パターンに倣った Supabase RPC
   - 姉妹関数 `updateStudentInfo` は既に Workers 化済みのため同型展開可能

2. **`requestAIAssistant`**（features.js）
