...
### AI 系（低頻度・複雑度高）は GAS 残置確定（Phase 6-C クローズ・2026-04-26）

Phase 6-C-06 完了時点で残った A 分類 7 件（全て Gemini API 系）について、**実運用上の使用頻度が低く・複雑度が高い**ことから「動いているものを壊さない」原則を優先し、**全て GAS 残置で確定**とする。Workers 化対象から正式に除外し、C 分類に再分類する：

| AI 関数 | 場所 | 役割 | 残置理由 |
|--------|------|------|---------|
| `requestAIAssistant` | features.js | 主力 AI チャット | 多データ依存（KB / Pricing / Schedule 等） |
| `executeAiAction` | features.js | AI アクション dispatcher | 複数 action 分岐の設計負荷 |
| `generateGradeAnalysis` | analysis.js | テスト全体 AI 分析生成 | 長時間処理（Workers CPU 上限懸念） |
| `generateAllAnalyses` | analysis.js | 一括 AI 分析（全テスト） | 同上・Admin 操作・低頻度 |
| `ocrAndSaveGradeSheet` | students.js | 成績シート OCR + 学生 lookup + 行毎書込 | 複雑度高・低頻度 |
| `parseGradeDataFromText` | students.js | 成績テキストパース + 学生 lookup + getDataSheetData | 同上 |
| `generateImageWithImagen` | features.js | Imagen 画像生成 | Imagen は Gemini helper 対象外・低頻度 |

これにより **未移行 A 分類は 0 件・Phase 6-C は完了**。今後 AI 系の挙動変更が必要になった場合は GAS 側で対応する。
...
| ocrAndSaveGradeSheet | 書込 | grades(Supabase), Gemini API | フロント: js-grades | GEMINI_API_KEY | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— 複雑度高・低頻度 |
| parseGradeDataFromText | 読取 | Gemini API | フロント: js-grades | GEMINI_API_KEY | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— 複雑度高・低頻度 |
...
| parseAndSaveAveragesFromText | 書込 | schoolAverages, Gemini API | フロント: js-grades-list | GEMINI_API_KEY | A | 中 | ✅ Workers 化済み（Phase 6-C-06）— `workers/src/gemini.js` 経由 + 内部呼出 saveSchoolAverages |
...
| generateGradeAnalysis | 書込 | testAnalysis, Gemini API | フロント: js-grades | GEMINI_API_KEY, GEMINI_API_KEY_BACKUP, GRADES_SIGMA_CONFIG | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— 長時間処理・低頻度 |
| generateStudentAnalyses | 書込 | studentAnalysis, Gemini API | GAS 内部のみ | GEMINI_API_KEY, GEMINI_API_KEY_BACKUP | C | | 内部ヘルパー |
| generateAllAnalyses | 書込 | testAnalysis, studentAnalysis, Gemini API | フロント: js-admin-ext | GEMINI_API_KEY, GEMINI_API_KEY_BACKUP | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— 一括長時間処理・Admin 操作 |
...
| requestAIAssistant | 書込 | Gemini API, 多くのデータ | フロント: js-admin-ext（AI ウィジェット） | GEMINI_API_KEY, GEMINI_API_KEY_BACKUP, AI_KNOWLEDGE_BASE, PRICING_TABLE_CONFIG | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— 多データ依存・複雑度高 |
| executeAiAction | 書込 | 多くのデータ | フロント: js-ai-actions | GEMINI_API_KEY 等 | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— dispatcher 設計負荷・低頻度 |
...
| parseLectureScheduleFromText | 書込 | Gemini API | フロント: js-lectures | GEMINI_API_KEY | A | 中 | ✅ Workers 化済み（Phase 6-C-05）— `workers/src/gemini.js` 経由 |
| generateImageWithImagen | 書込 | Gemini/Imagen API | フロント: js-lectures-imagen | GEMINI_API_KEY | **C** | — | 🔵 **GAS 残置確定**（Phase 6-C クローズ・2026-04-26）— Imagen API は Gemini helper 対象外・低頻度 |
...
| ~~transcribeAndSummarizeAudio~~ | — | — | — | — | 削除済 | — | 🗑️ 議事録 AI 機能廃止により削除（2026-04-26）— UI もテキスト直接入力のみに簡素化 |
| ~~mergeTranscriptsAndSummarize~~ | — | — | — | — | 削除済 | — | 🗑️ 議事録 AI 機能廃止により削除（2026-04-26）— 同上 |
...
| 分類 | 件数 | 内訳 |
|------|------|------|
| ✅ Workers 化済み | 約 115 | 旧台帳掲載 99 + 旧台帳未掲載 16 |
| **未移行 A** | **0** | 🎉 全件移行・残置・廃止のいずれかに分類済（Phase 6-C 完了） |
| 🔵 AI 系 GAS 残置確定（Phase 6-C クローズ） | 7 | requestAIAssistant, executeAiAction, generateGradeAnalysis, generateAllAnalyses, ocrAndSaveGradeSheet, parseGradeDataFromText, generateImageWithImagen |
| 🗑️ 機能廃止により削除済 | 2 | transcribeAndSummarizeAudio, mergeTranscriptsAndSummarize（議事録 AI 文字起こし・要約・2026-04-26） |
...
### 旧「未移行 A 分類」の最終処遇（Phase 6-C クローズ・2026-04-26）

Phase 6-C-06 完了時点で残った 7 件は全て **「C 残置確定」に再分類**。Workers 化対象から正式に除外。詳細は冒頭「重要方針」セクション参照。

| 関数 | 場所 | 残置理由 | 旧分類 |
|------|------|---------|--------|
| `requestAIAssistant` | features.js | 主力 AI チャット・多データ依存 | A 高 |
| `executeAiAction` | features.js | dispatcher 設計負荷・低頻度 | A 高 |
| `generateGradeAnalysis` | analysis.js | 長時間処理・低頻度 | A 中 |
| `generateAllAnalyses` | analysis.js | 一括長時間・Admin 操作 | A 中 |
| `ocrAndSaveGradeSheet` | students.js | 学生 lookup + 行毎書込・複雑 | A 中 |
| `parseGradeDataFromText` | students.js | 同上・複雑 | A 中 |
| `generateImageWithImagen` | features.js | Imagen は Gemini helper 対象外・低頻度 | A 低 |
...
- ✅ **Phase 6-C-05**（テキストパース）— `parseLectureScheduleFromText` 移行（兄弟関数の同型展開）
- 🗑️ **議事録 AI 廃止**（2026-04-26）— `transcribeAndSummarizeAudio` / `mergeTranscriptsAndSummarize` を機能ごと削除（フロント UI もテキスト直接入力のみに簡素化）
- ✅ **Phase 6-C-06**（テキストパース第2弾）— `parseAndSaveAveragesFromText` 移行（`ocrAndExtractAverages` 兄弟関数の同型展開）
- 🏁 **Phase 6-C クローズ**（2026-04-26）— 残った A 分類 7 件（AI 系全件）を「C 残置確定」に再分類。**未移行 A は 0 件**。Workers 化対象は完了。詳細は冒頭「重要方針」セクション参照。

### 今後の方針

**Workers 化フェーズは Phase 6-C をもって完了**。今後は以下の方針で運用：

| 観点 | 方針 |
|------|------|
| 新機能の追加 | 原則 Workers 側に実装（GAS 専用 API・トリガー必要時のみ GAS） |
| AI 系の挙動変更 | GAS 側で対応（C 残置確定のため） |
| トリガー系の追加変更 | GAS 側で対応（時間トリガーは GAS 確定） |
...
| AI 系（低頻度・複雑度高・Phase 6-C クローズ・2026-04-26 確定） | `requestAIAssistant` / `executeAiAction` / `generateGradeAnalysis` / `generateAllAnalyses` / `ocrAndSaveGradeSheet` / `parseGradeDataFromText` / `generateImageWithImagen` |
