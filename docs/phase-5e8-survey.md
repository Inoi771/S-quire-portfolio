...
> ### 【解消済み: Phase 5-E-10（コミット `cf4c8bc`）にて実装】
>
> 本セクションで挙げた「Workers 経由では `operationLogs` に監査ログが残らない」
> 宿題は Phase 5-E-10 で案 B 寄りの形（本ファイル対象 2 関数専用の共通ヘルパー
> `logAdminActionToFirestore()` を `workers/src/functions/schedule-overrides.js`
> 内に新設）で解消済み。
>
> **解消箇所**
> - `workers/src/functions/schedule-overrides.js:50-65` — `logAdminActionToFirestore()` ヘルパー
> - `workers/src/functions/schedule-overrides.js:395` — `setLectureDeadlineOverride` 内で呼び出し
> - `workers/src/functions/schedule-overrides.js:415` — `deleteLectureDeadlineOverride` 内で呼び出し
>
> Phase 5-E-11（コミット `f8c7b0c`）では同様の共通化を Admin 3 関数（`workers/src/functions/admin.js` の `recordOperationLog_`）にも適用済み。両者は現時点で別実装だが、Admin 書込ラッパー全般を共通化する判断は将来フェーズで行う。
>
> 以下の記述は履歴として残すが、現状の挙動表（ Workers 経由で `operationLogs` が残らない）は古く、現在は **Workers ルート・GAS ルート双方で `operationLogs` に残る**。
...
- **グループ C の `operationLogs` 書込欠落** — **✅ Phase 5-E-10（`cf4c8bc`）で解消済み**。`workers/src/functions/schedule-overrides.js` に `logAdminActionToFirestore()` ヘルパーを新設し、`setLectureDeadlineOverride` / `deleteLectureDeadlineOverride` から呼ぶことで Workers ルートでも `operationLogs` に監査ログが残る。詳細は本ドキュメント「5-E-10 への宿題」セクション先頭の解消記録を参照。
