...
> **ステータス（2026-04-26 時点）**: 移行フェーズは Phase 6-C をもって完了・クローズ。
> Workers 化済み 約115 関数 / GAS 残置確定 約80 関数（トリガー・GAS専用API・低頻度AI系）/ 未移行A 0 件。
> 残り約80関数は「未移行」ではなく意図的な GAS 残置。詳細は末尾「全体集計（2026-04-26 最終）」参照。
...
| ステップ | 誰が | 内容 |
|---------|------|------|
| ① | **オーナー手動** | `ALTER TABLE staffs ADD COLUMN lec_grades JSONB DEFAULT '[]'::jsonb;` |
| prep-a | Claude | `migrate-lec-grades.js` のみをコミット（staffFromSupabase_ 等の変更は含めない） |
| — | **オーナー手動** | GAS エディタで `migrateLecGradesDryRun()` → 確認 → `migrateLecGrades()` 実行 |
...
| 2 | Network タブ | `s-quire-api.YOUR_CLOUDFLARE_SUBDOMAIN.workers.dev` <!-- 要設定 --> への POST が 200 |
...
Phase 5: GASバックエンドをCloudflare Workersへ移行    ✅ 完了
Phase 6-A: C 分類関数の Workers 移行                  ✅ 完了
Phase 6-B: デプロイ設定の切替（旧 Phase 6）           ✅ 完了
Phase 6-C: B/A 分類関数の Workers 移行                ✅ 完了（2026-04-26 クローズ）
```
...
#### ステップ1: Supabase Storageバケット作成（オーナーが実施）
...
1. GitHub リポジトリ `YOUR_GITHUB_ACCOUNT/S-quire` <!-- 要設定 --> を開く
...
### Claude が実施する作業（オーナーの Step 1〜4 完了後）

オーナーから「**KV の Namespace ID**」を教えてもらったら以下を実施する。
...
[[kv_namespaces]]
binding = "KV"
id = "（オーナーが教えてくれた KV Namespace ID）"
preview_id = "（同じIDでOK）"
```
...
| 確認項目 | 結果 |
|---------|------|
| Workers URL でレスポンスが返る | ✅ https://s-quire-api.YOUR_CLOUDFLARE_SUBDOMAIN.workers.dev <!-- 要設定 --> |
| GitHub Actions が成功している | ✅ 緑チェック確認済み |
| KV が wrangler.toml に設定されている | ✅ ID: YOUR_KV_NAMESPACE_ID <!-- 要設定 --> |
| GAS・アプリが従来通り動作している | ✅ 確認済み |

---

## Phase 5-B: Workers 基盤構築 → 段階的関数移行（並行稼働方式）✅ 完了
...
```javascript
// 追加する2定数
var WORKERS_URL = 'https://s-quire-api.YOUR_CLOUDFLARE_SUBDOMAIN.workers.dev';  // 要設定
...
export default {
  async fetch(request, env) {
    ...
    const CORS = { 'Access-Control-Allow-Origin': 'https://YOUR_FIREBASE_PROJECT_ID.web.app' };  // 要設定
...
| B-③ | `workers/src/router.js` + `workers/src/index.js` 更新（`ping` エンドポイント追加・CORS対応） | `curl -X POST https://s-quire-api.YOUR_CLOUDFLARE_SUBDOMAIN.workers.dev` <!-- 要設定 --> で `{"status":"ok"}` が返ること |
...
# 追加する [vars] セクション（公開値）
[vars]
FIREBASE_WEB_API_KEY = "YOUR_FIREBASE_WEB_API_KEY"  # 要設定（Firebase コンソール → プロジェクト設定 → ウェブ API キー）
...
> **オーナー作業が必要**: これらは Claude がコミットで追加できない。事前に手動で追加してもらう必要がある。
...
| ステップ | 誰が | 内容 |
|---------|------|------|
| **事前①** | **オーナー手動** | Cloudflare Dashboard で 4 シークレット追加（GEMINI_API_KEY, GEMINI_API_KEY_BACKUP, APP_FOLDER_ID, ACCESS_FOLDER_ID） |
...
- `6bd4278` Phase 6-C-02: submitStudentInfo を Workers 化

---

# Phase 6-C-03 完了記録（2026-04-26）

## 概要

**A 分類「優先度: 中」の `ocrAndExtractAverages` を Workers 化。**

Gemini API 呼出系の Workers 移行第 1 弾。Phase 6-B-02 で整備した `workers/src/gemini.js` の本番実績を作る目的を兼ねる。残り Gemini 系 11 件の移行で同じパターンを再利用するため、橋頭堡的フェーズ。

## 移行した関数

| 関数 | GAS 場所 | 役割 |
|------|---------|------|
| `ocrAndExtractAverages` | `students.js:1686-1743` | 学校別平均点画像（base64）を Gemini OCR で読み取り Supabase に保存（成績管理 > 一覧表 > 学校別平均点 OCR） |
...
| プロンプト | 完全一致（OCR 用 prompt） | 同じ |
| API キー取得 | `getProperty(PROP_KEYS.GEMINI_API_KEY)` | `fetchGeminiWithRetry` 内で `env.KV.get('prop:GEMINI_API_KEY')` |
| リトライ | `fetchGeminiWithRetry_` | `fetchGeminiWithRetry`（同型） |
| エラー文言 | 「Gemini APIキーが設定されていません…」「平均点データが読み取れませんでした」「AIからの応答がありませんでした」 | 完全一致 |
| 内部呼出 | `saveSchoolAverages(year, testName, extracted, true)` | 同モジュール内の `saveSchoolAverages` を直接 await（router 経由しない） |

## 変更ファイル（3 ファイル・1 コミット）

| ファイル | 変更内容 |
|---------|---------|
| `workers/src/functions/students.js` | import に `fetchGeminiWithRetry` / `extractGeminiText` / `parseGeminiErrorMessage` 追加。`saveSchoolAverages` の直後に `ocrAndExtractAverages` を追加。+85 行 |
| `workers/src/router.js` | import + HANDLERS マップに登録 |
| `gas-bridge.html` | `WORKERS_FUNCTIONS` Set に `'ocrAndExtractAverages'` を追加 |

## GAS 版との差分

**なし（完全等価）**:
- プロンプト・モデル名・generationConfig 完全一致
- エラーメッセージ完全一致（API キー未設定 / AI 応答なし / データ読取不能）
- 戻り値形状 `{success, savedCount, updatedCount, extracted}` 完全一致
- `skipExisting=true` で既存値保持の挙動も同じ

## 動作確認チェックリスト

- [ ] 成績管理タブ → 一覧表サブタブ → 「学校別平均点」セクション → OCR 画像アップロード → 「Gemini で読み取る」ボタン
- [ ] 解析結果プレビュー表示 → 学校名・教科別平均点が正しく抽出されている
- [ ] 「保存」ボタン押下後、savedCount / updatedCount が画面に表示される
- [ ] 既存データを skipExisting で保持する挙動（既存行は updatedCount=0）
- [ ] 不鮮明な画像で「平均点データが読み取れませんでした」エラー表示
- [ ] API キー未設定（KV 未投入）時に「Gemini APIキーが設定されていません…」エラー

## ロールバック手順

`gas-bridge.html` の `WORKERS_FUNCTIONS` から `'ocrAndExtractAverages'` を 1 行除外して Hosting 再デプロイすれば即座に GAS 経路へロールバック可能。GAS 版（`students.js:1686-1743`）はフォールバック保険として残置。

## A 分類の進捗

| 状態 | 件数 |
|------|------|
| Workers 化済 | 約 112（旧台帳掲載 96 + 旧台帳未掲載 16） |
| 未移行 A | **12**（うち Gemini API 系 全 12 件） |
| 残存「高」優先度 | 2（`requestAIAssistant` / `executeAiAction`） |

## 次フェーズ候補

- **Phase 6-C-04 候補（Gemini 系の連続移行）**: `parseGradeDataFromText` / `parseLectureScheduleFromText` / `ocrLectureSchedule` 等
...
- `e00df30` Phase 6-C-05: parseLectureScheduleFromText を Workers 化

---

# Phase 6-C-06 完了記録（2026-04-26）

## 概要

**A 分類「優先度: 中」の `parseAndSaveAveragesFromText` を Workers 化。**

Phase 6-C-03 で移行した `ocrAndExtractAverages` の兄弟関数（テキスト入力版）。同型展開で連続 Gemini 移行第 4 弾（OCR/Parse 系のうち 4 件目）。

## 移行した関数

| 関数 | GAS 場所 | 役割 |
|------|---------|------|
| `parseAndSaveAveragesFromText` | `students.js:1522-1574` | 学校別平均点のテキスト（ウェブからのコピペ等）を Gemini で解析し Supabase に保存 |

## 設計ポイント

`ocrAndExtractAverages`（Phase 6-C-03）と同じパターン。違いは:

| 項目 | ocrAndExtractAverages | parseAndSaveAveragesFromText |
|------|----------------------|----------------------------|
| 入力 | base64Image + mimeType + 2 引数（計 4 引数） | text + 3 引数（計 4 引数） |
| Gemini parts | `[{inline_data}, {text:prompt}]` | `[{text:prompt}]`（テキストのみ） |
| プロンプト | OCR 用（画像読取） | テキスト解析用（ウェブコピペ前提） |
| プロンプト末尾 | （画像を別 part で受ける） | `対象テキスト:\n` + text を連結 |
| skipExisting 引数 | 内部固定で true（ハードコード） | フロント指定値を使用（`skipExisting !== false` でデフォルト true） |
| 内部呼出 | 同じく `saveSchoolAverages([y,t,extracted,true])` | `saveSchoolAverages([y,t,extracted, skipExisting !== false])` |

その他（モデル名・generationConfig・エラーメッセージ・戻り値形状）は完全一致。

## 変更ファイル（3 ファイル・1 コミット）

| ファイル | 変更内容 |
|---------|---------|
| `workers/src/functions/students.js` | `ocrAndExtractAverages` の直後に `parseAndSaveAveragesFromText` を追加。+78 行 |
| `workers/src/router.js` | import + HANDLERS マップに登録 |
| `gas-bridge.html` | `WORKERS_FUNCTIONS` Set に `'parseAndSaveAveragesFromText'` を追加 |

## GAS 版との差分

**なし（完全等価）**:
- プロンプト・モデル・generationConfig 完全一致
- エラーメッセージ完全一致（API キー未設定 / AI 応答なし / 平均点読取不能）
- 戻り値形状 `{success, savedCount, updatedCount, extracted}` 完全一致
- skipExisting のデフォルト挙動も `!== false` で一致

## 動作確認チェックリスト

- [ ] 成績管理タブ → 一覧表サブタブ → 「学校別平均点」セクション → 「テキストから読み込む」エリアにウェブコピペ → 「読み込む」ボタン
- [ ] 解析結果が表示される（学校名・5 教科平均点）
- [ ] 「保存」後に savedCount / updatedCount が表示される
- [ ] 既存データが skipExisting=true で保持される（既存行は updatedCount=0）
- [ ] 解析不能テキストで「平均点データが読み取れませんでした」エラー
- [ ] API キー未設定時に「Gemini APIキーが設定されていません…」エラー

## ロールバック手順

`gas-bridge.html` の `WORKERS_FUNCTIONS` から `'parseAndSaveAveragesFromText'` を 1 行除外して Hosting 再デプロイすれば即座に GAS 経路へロールバック可能。GAS 版（`students.js:1522-1574`）はフォールバック保険として残置。

## A 分類の進捗

| 状態 | 件数 |
|------|------|
| Workers 化済 | 約 115（旧台帳掲載 99 + 旧台帳未掲載 16） |
| 未移行 A | **7**（うち Gemini API 系 全 7 件） |
| 残存「高」優先度 | 2（`requestAIAssistant` / `executeAiAction`） |
| 議事録 AI 廃止により削除 | 2（`transcribeAndSummarizeAudio` / `mergeTranscriptsAndSummarize`） |
...
オーナー判断により、Workers 化フェーズは Phase 6-C をもって正式終了。今後の AI 系の挙動変更は GAS 側で対応する。
