...
設計や開発だけでなく、運用中のトラブル対応も同じ分業で進めています。例えば、GitHub Actions の自動マージが「Your account is suspended」というエラーで失敗した時、私はエラーログのスクリーンショットを Claude に共有し、過去の自動マージ履歴と照らし合わせ、ダミーコミットで再現テストを行うという手順を Claude と相談しながら組み立て、最終的に「一過性のエラー」と判断するところまでたどり着きました。コードは読めなくても、症状を正確に観測し、AIに過不足なく情報を渡し、検証手順を一緒に設計することで、AI単独では到達できない解決に届きます。「動いているシステムを止めない」ためのこの動き方こそ、私が運用で価値を出している部分です。
