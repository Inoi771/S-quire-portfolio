...
徳島県の学習塾「個別指導スクエア」で実運用中の業務管理システム。
一人の塾講師が、AI（Claude / Gemini）を活用して設計・運用しています。

## 3行で言うと

- 学習塾8校舎で実運用中の業務管理システムを、実装経験のない塾講師がAIと協働して構築・運用
- Claude（設計）と Claude Code（実装）、Gemini API（プロダクト内AI機能）を組み合わせた「設計・運用主導型」の開発体制
- 実装力ではなく、「AIに何を作らせ、どう壊さず運用するか」を重視
...
> **お時間がない方へ**: 「[開発体制：設計と実装の分業](#開発体制設計と実装の分業)」「[AIを制御する仕組み — CLAUDE.md](#aiを制御する仕組み--claudemd)」「[設計→実装のワークフロー — SNS-DESIGN.md を例に](#設計実装のワークフロー--sns-designmd-を例に)」の3セクションが、このポートフォリオの中核です。
...
業務システムの内製は2024年初頭から、実装経験のない立場でAIと組んで取り組んできました。
...
当時のAIは文脈長が短く、数ターンで会話の流れを忘れてしまうため、欲しい関数を一つひとつ依頼して、生成されたコードを自分で貼り合わせていく形でした。エラーや修正箇所も自分で見つけて、再度AIに尋ねながら直していました。**処理の流れを追い、原因箇所の見当をつけてAIに正しく質問するスキル**は、この時期に基礎を築きました。生徒・スタッフの利用減少とRenderの有料化を受け、2026年3月で運用を終了しています。

**2024年〜2025年：englishtest アプリ**

並行して、英単語学習アプリ **englishtest** も開発・運用していました。Google Cloud TTS API で音声を生成し、GitHub 上に音声ファイルをホストする構成で、PWA 対応・同音異義語の発音記号対応など、塾の英語指導の現場ニーズに合わせて細かく調整しました。S-quire ほど大規模ではありませんが、「現場で使われる小さなツールを、本人が運用しながら育てる」という今のスタイルの原型は、このアプリで固まりました。
...
私（猪井 真二／@Inoi771）は、実装経験のない立場から、AIとの分業によってこのシステムを構築・運用しています。
...
**この設計書は、私とClaudeの対話から生まれたものです。** 塾の業務知識・既存システムの構造・運用上の制約をすべて頭に入れた上で、AIと議論しながらこの粒度まで落とし込んでいます。文章化はAIが担いますが、Phase分割の判断、無料運用の選択、優先順位の決定は私の仕事です。
...
- **[CLAUDE.md](./CLAUDE.md)** — Claude Code が作業時に守るルール集。「AIに何を書かせないか」を明文化した制御装置
- **[SNS-DESIGN.md](./SNS-DESIGN.md)** — 未実装機能（Instagram連携）の設計書。「AIに何を書かせるかを決める」工程の実例
