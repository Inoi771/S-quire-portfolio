...
デプロイ後、教師用ページの「⚙️ 設定」タブ →「新年度の作成」セクションで年度名（例:「2026年度版」）を入力し「📁 年度フォルダを作成」ボタンを押す。

以下が自動で作成される:
- 年度フォルダ（例:「2026年度版」）
- 教科書スプレッドシート（NewHorizon・Sunshine・入試対策編）
- 各学年シート（中学1年・中学2年・中学3年）
- レッスン順序シート
- 入試対策用シート（通常・不規則動詞①・不規則動詞②）

> 入試対策編は既存の最新年度からデータごとコピーされます（既存年度がない場合は空で作成）。
...
教師用ページの「⚙️ 設定」タブ →「新年度の作成」セクションで年度名を入力し「📁 年度フォルダを作成」ボタンを押すだけ。

必要なフォルダ・スプレッドシート・シートがすべて自動作成される（入試対策編は最新年度からデータごとコピー）。
