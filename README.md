...
> **お時間がない方へ**: 「[開発体制：設計と実装の分業](#開発体制設計と実装の分業)」と「[設計→実装のワークフロー — SNS-DESIGN.md を例に](#設計実装のワークフロー--sns-designmd-を例に)」の2セクションが、このポートフォリオで最もお見せしたい部分です。
...
業務システムの内製は2024年初頭から、未経験の状態でAIと組んで取り組んできました。
...
私（猪井 真二／@Inoi771）は、プログラミング未経験の非エンジニアです。

その立場でこのシステムを構築・運用できているのは、AIとの徹底した分業によります。実装は Claude Code が担い、私は業務知識・設計判断・優先順位付け・運用判断に集中する役割分担です。
