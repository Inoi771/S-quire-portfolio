...
機密情報は置換・除去済みですが、設計判断・運用記録・コードベースの規模感は加工せずそのままです。本番運用されているのは別アカウントの S-quire で、このリポジトリは公開用のコピーです。
...
私自身はコードを読めないため、AIが「ついでに周辺を直す」「動いているコードを勝手にリファクタする」といった暴走をすると気付けません。CLAUDE.md は、本番運用中のシステムを一人で保守するための制御装置であり、安全装置でもあります。
