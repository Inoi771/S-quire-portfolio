...
## AIを制御する仕組み — CLAUDE.md 抜粋

リポジトリのルートに置いた `CLAUDE.md` には、Claude Code が作業時に必ず守るべきルールを記載しています。Claude Code はセッション開始時にこのファイルを自動で読み込むため、私が毎回指示しなくても、ここに書いたルールに従って動作します。

私自身はコードを読めないため、AIが「ついでに周辺を直す」「動いているコードを勝手にリファクタする」といった暴走をすると気付けません。CLAUDE.md はそれを未然に防ぐための制御装置です。本番運用中のシステムを一人で保守するための、私なりの安全装置でもあります。
...
> **守るべき品質ルール**: 関数を長くしない・グローバル変数を増やさない・エラーを握りつぶさない

「私はコードを読めない」という事実を、AIの責務として明文化しています。私がレビューできない以上、コード品質はAI側が黙って守るしかありません。このルールは、私とAIの分業関係を端的に表しています。
...
8校舎のスタッフが日常使うシステムを守るための、極めて保守的な設計哲学です。AIは放っておくと既存コードを「より良く」書き換えたがりますが、本番運用中の塾業務では「動いているものを壊さない」が最優先です。

### 3. プッシュ前チェックリスト

> コードをプッシュする前に、以下を必ず確認する：
>
> - [ ] `git fetch origin && git merge origin/main` を実行して最新の main を取り込んだか？
...
複数の `claude/` ブランチが並行してデプロイする運用のため、main の取り込み忘れで他ブランチの変更が消える事故を防ぐチェックリストです。AIに任せきりにせず、運用上のハマりどころを自分で見抜いて仕組み化した例です。

---

このような運用ルールは CLAUDE.md に他にも多数記載しており、新しい設計判断や運用上の気付きが出るたびに追記しています。「AIに何を書かせるか」だけでなく「AIに何をさせないか」を決めることも、私の重要な仕事です。
