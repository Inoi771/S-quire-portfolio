...
### ❌ パターン11: Firebase デプロイ時の Firestore Rules 関連エラー（繰り返し発生）

**発生した問題**: 「Missing or insufficient permissions」エラーが繰り返し発生。

**根本原因**: Firestore セキュリティルールは Firebase コンソールで管理されており、
CI/CD のサービスアカウントには `firebaserules.googleapis.com` の権限がない。
そのため CI から Firestore Rules をデプロイしようとすると **403 エラーでデプロイ全体が失敗**する。
...
```yaml
# ❌ firestore.rules を paths に含めない（Hosting デプロイが無駄に走る）
paths:
  - 'firestore.rules'
```
...
```javascript
// ❌ firebase-init.html で Firestore SDK のプロトタイプを書き換える
// enablePersistence との干渉で予期しないエラーが発生する可能性あり
Object.getPrototypeOf(fbDb.collection('_').doc('_')).get = wrappedGet;
```
...
**チェックポイント**:
- `deploy-firebase.yml` の deploy コマンドは **`--only hosting` のみ**。絶対に `firestore:rules` を含めない（403 エラーでデプロイ全体が失敗する）
- `firestore.rules` を `deploy-firebase.yml` の `paths` トリガーに含めない
- Firestore Rules を変更する必要がある場合は **Firebase コンソールから手動で**デプロイすること
- `firebase-init.html` で Firestore SDK 内部のプロトタイプ（`Query.prototype.get` 等）を書き換えない
- 「Missing or insufficient permissions」が出たら、まず Firebase コンソールでルールを確認すること
